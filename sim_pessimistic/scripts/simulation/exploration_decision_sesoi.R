...
n_exp <- 10 # number of experiments we run in 1st stage (exploration)
ES_true <- ES_data_Carneiro$ES_d # empirical effect sizes (ES)
...
# decision to go on
# this decision depends on whether our SESOI is within the 95 % CI around exploratory effect size 
...
dat$ES_true <- rep(current_ES, 2)

# save data frame
# enter SESOI used as decision criterion
# write.csv(dat, file = "./data/exploratory_data_sesoi_??")
