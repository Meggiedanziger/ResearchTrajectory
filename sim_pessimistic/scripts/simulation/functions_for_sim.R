...
get_summary_study <- function(study_data) {
  ...
  hedges  <- hedges_g(values ~ treatment, data = study_data,
                      correction = 1,
                      pooled_sd = FALSE,
                      paired = FALSE,
                      ci = 0.95,
  ...
  study_summary <-
    ...
    mutate(t_value = round(t$statistic, 4),
           ...
           hedge = round(hedges$Hedges_g, 4),
           ci_low = round(hedges$CI_low, 4),
           ci_high = round(hedges$CI_high, 4))
  
  study_summary <-
    study_summary %>%
    group_by(sample_size, study_id, t_value, p_value) %>%
    summarize(hedge = mean(hedge),
              ci_low = mean(ci_low),
...
get_summary_study_rep_BF <- function(study_data) {
  ...
  BF <- ttestBF(x = study_data$values[study_data$treatment == "atreat"],
...
get_decision_equiv <- function(exploratory_data_summary,
  ...
  helper <-
    cbind(exploratory_data_summary$ci_low,
          exploratory_data_summary$ci_high)
  
  # select_experiments <- apply(helper, 1, function(x) {min(x)} < SESOI)
  
  select_experiments <- apply(helper, 1, 
                              function(x) {
                                
                                min(x) <= SESOI & max(x) >= SESOI | min(x) >= SESOI
...
calc_sample_size <- function(data, sample_size, max_sample_size = 200,
  ...
  if (method == 1) {
    es_measured <- abs(aa$hedges) # take initial study main effect
