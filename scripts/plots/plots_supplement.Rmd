...
```{r ES distribution, echo = FALSE, message = FALSE, results = "hide"}
...
```{r sifigure1, echo = FALSE, fig.align = 'center', fig.cap = '\\textsf{\\textbf{Transition rates for two different confidence intervals and SESOI.} \\textbf{a}, Given a SESOI of 0.5 and a 50 $\\%$ CI, 7209 effect sizes > 0 transition to the confirmatory stage. Employing an 80 $\\%$ CI increases the number to 8332 which is the same as for a 95 $\\%$ CI. \\textbf{b}, Given a SESOI of 1.0 and a 50 $\\%$ CI, 4243 effect sizes > 0 transition to confirmation. Employing an 80 $\\%$ CI increases the number to 5790 which is still appreciably less than for a 95 $\\%$ CI. This showcases that depending on the SESOI used the bounds of the CI may have a crucial effect on the transition rate.}'}
...
```{r sifigure2, echo = FALSE, message = FALSE, out.width = '95%', fig.align = 'center', fig.cap = '\\textsf{\\textbf{Outcomes across SESOI trajectory for two different confidence intervals and SESOI.} \\textbf{a}, A narrower CI increases the PPV for larger SESOI. \\textbf{b}, The NPV is slightly decreased if a 50 $\\%$ CI is employed. \\textbf{c}, The FPR varies as a function of CI and SESOI. A narrower CI increasingly reduces the FPR as SESOI increases. \\textbf{d}, The FNR increases for narrower CI as SESOI increases.}'}
...
```{r sifigure12, echo = FALSE, fig.align = 'center', fig.cap = '\\textsf{\\textbf{Pessimistic effect size distribution and transition rates.} \\textbf{a}, Distribution of empirical effect sizes (\\textit{n} = 336) extracted from Carneiro et al., 2018. The study systematically examined effect sizes in the rodent fear conditioning literature. The effect sizes range from -4.14 to 2.6, with a median of -0.37. \\textbf{b-d}, Exploratory effect sizes (\\textit{n} = 10000). The shaded bars show the effect sizes that were identified for confirmation using one of the two decision criteria. \\textbf{b}, Yellow shaded bars indicate those effect sizes that were identified for confirmation using a significance threshold of 0.05. \\textbf{c}, Dark blue shaded bars indicate effect sizes that were selected using a SESOI of 0.5. \\textbf{d}, Light blue shaded bars indicate effect sizes that were selected using a SESOI of 1.0.}'}
...
ggdraw(xlim = c(0, 2),
       ylim = c(0, 2)) +
  draw_plot(ES_histrogram_Carneiro,  0, 1, 1, 1) +
  draw_plot(ES_histrogram_Carneiro_selected_sig, 1, 1, 1, 1) +
  draw_plot(ES_histrogram_Carneiro_selected_sesoi_0.5, 0, 0, 1, 1) +
  draw_plot(ES_histrogram_Carneiro_selected_sesoi_1.0, 1, 0, 1, 1) +
  draw_plot_label(c("a", "b", "c", "d"),
                  c(0, 1, 0, 1),
                  c(2, 2, 1, 1),
                  size = 9)
...
```{r sifigure13, echo = FALSE, message = FALSE, out.width = '95%', fig.align = 'center', fig.cap = '\\textsf{\\textbf{Sample sizes in confirmation and outcomes across trajectory based on pessimistic effect size distribution.} \\textbf{a}, Number of animals needed in the first confirmatory study. In the Standard trajectory, sample sizes are low, as they are based on large exploratory effect sizes. Error bars represent standard deviations. In case of trajectories using a SESOI, the number of animals is fixed. All sample sizes displayed and reported in the text are the number of animals needed in \\textit{each} group (control and intervention). \\textbf{b}, The positive predictive value across the trajectory is consistently above the prior probability (dotted lines) only in the SESOI trajectory. \\textbf{c}, The false positive rate across the trajectory is consistently lower in the Standard trajectory. \\textbf{d}, The false negative rate across the trajectory varies as a function of trajectory and SESOI.}'}
...
ggdraw(xlim = c(0, 2),
       ylim = c(0, 2)) +
  draw_plot(plot_N,  0, 1, 1, 1) +
  draw_plot(plot_PPV, 1, 1, 1, 1) +
  draw_plot(plot_FPR, 0, 0, 1, 1) +
  draw_plot(plot_FNR, 1, 0, 1, 1) +
  draw_plot_label(c("a", "b", "c", "d"),
                  c(0, 1, 0, 1),
                  c(2, 2, 1, 1),
                  size = 9)
