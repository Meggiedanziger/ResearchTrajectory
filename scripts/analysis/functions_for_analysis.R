...
# function to compute outcomes after exploration in the SESOI trajectory 
# with varying confidence intervals (CI)

compute_outcomes_exploration_CI <- function() {
  
  selected <-
    dat_ci %>% 
    group_by(CI, SESOI, init_sample_size) %>%
    filter(selection == 1) %>% 
    summarize(selected = n())
  ...
  # add columns for positives and negatives to calculate outcomes
  dat_ci$all_positives <- rep(c(mat[2, 3], 
                                mat[2, 5],
                                mat[2, 3], 
                                mat[2, 5]),
                              each = 4)
  
  dat_ci$all_negatives <- rep(c(mat[3, 3], 
                                mat[3, 5],
                                mat[3, 3], 
                                mat[3, 5]),
                              each = 4)
...
# function to compute outcomes for second stage only in the SESOI trajectory
# with varying confidence intervals (CIs)

compute_outcomes_rep_CI <- function() {
...
compute_outcomes_across_trajectory_CI <- function() {
  ...
  # add true negatives from exploration
  dat$true_neg  <- dat$true_neg + outcomes_sesoi_exp_CI$true_neg
  
  # add false negatives from exploration
  dat$false_neg <- dat$false_neg + outcomes_sesoi_exp_CI$false_neg
  
  # add columns for positives and negatives to calculate outcomes
  dat$all_positives <- rep(c(mat[2, 3], 
                             mat[2, 5],
                             mat[2, 3], 
                             mat[2, 5]),
                           each = 4)
  
  dat$all_negatives <- rep(c(mat[3, 3], 
                             mat[3, 5],
                             mat[3, 3], 
                             mat[3, 5]),
                           each = 4)
...
# function to compute outcomes across trajectory in the SESOI trajectory
# for 80 % power for confirmatory sample size calculation

compute_outcomes_across_trajectory_high_power <- function() {
  ...
  # add columns for positives and negatives to calculate outcomes
  dat$all_positives <- rep(c(mat[2, 2], 
                             mat[2, 3], 
                             mat[2, 4], 
                             mat[2, 5]),
                           each = 4)
  
  dat$all_negatives <- rep(c(mat[3, 2], 
                             mat[3, 3], 
                             mat[3, 4], 
                             mat[3, 5]),
                           each = 4)
...
# function to compute outcomes for exploratory stage in the Standard trajectory
# with 30 experimental units at exploratory stage

compute_outcomes_exploration_standard_30eu <- function() {
...
compute_outcomes_rep_standard_30eu <- function() {
...
compute_outcomes_across_trajectory_standard_30eu <- function() {
  ...
  # add true negatives from exploration
  dat$true_neg  <- dat$true_neg + outcomes_standard_exp_30eu$true_neg
  
  # add false negatives from exploration
  dat$false_neg <- dat$false_neg + outcomes_standard_exp_30eu$false_neg
